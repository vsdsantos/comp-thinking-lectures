...
# ╔═╡ 83c60eee-2ce0-11eb-1c91-7bb403d4749c
(before=size(img), after=size(remove_in_each_row_no_vcat(img, 1:size(img, 1))))
...
function greedy_seam(energies, starting_pixel::Int)
	...
	for i in 1:m-1
		...
		les = [energies[i+1, k] for k in j1:j2]
...
# ╔═╡ d9327a06-2ce1-11eb-2616-37d15c93f1ac
let
	A = rand(Float64, 3, 3)
	
	greedy_seam(A, 2), Gray.(A)
end
...
function least_energy(energies, i, j)
	...
	les = [least_energy(energies, i+1, j)[1] for j in j1:j2]
...
# ╔═╡ 2cb62d60-5c50-11eb-007c-0f20240f21e4
function recursive_seam2(energies, starting_pixel)
	m, n = size(energies)
	...
	energies_crop = @view energies[2:end,:]
	
	j1, j2 = max(1, j-1), min(j+1, n)
	
	sub_idxs = [recursive_seam(energies_crop, k) for k in j1:j2]
	
	sub_energies = []
	for k in sub_idxs
		le = []
		for (i,j) in enumerate(k)
			push!(le, energies_crop[i,j])
		end
		push!(sub_energies, le)
	end
	
	# @show sub_energies
	min_e, min_sub_idx = findmin(map(a->sum(a), sub_energies))
	
	idx = sub_idxs[min_sub_idx]
...
function memoized_least_energy(energies, i, j, memory)
	...
	les = Array{Float64,1}(undef, j2-j1+1)
	for (idx,k) in enumerate(j1:j2)
		if haskey(memory, (i+1, k))
			les[idx] = memory[(i+1, k)]
		else
			e = memoized_least_energy(energies, i+1, k, memory)[1]
			les[idx] =  e
			memory[(i+1, k)] = e
		end
	end
...
function recursive_memoized_seam(energies, starting_pixel)
	...
	for i in 1:m-1
		S, j = memoized_least_energy(energies, i, j, memory)
		push!(seam, j)
	end
...
function matrix_memoized_least_energy(energies, i, j, memory)
	...
	for (idx,k) in enumerate(j1:j2)
		if memory[i+1, k] != 0
			...
			e = matrix_memoized_least_energy(energies, i+1, k, memory)[1]
			les[idx] =  e
			memory[i+1, k] = e
		end
	end
...
function matrix_memoized_seam(energies, starting_pixel)
	...
	for i in 1:m-1
		S, j = matrix_memoized_least_energy(energies, i, j, memory)
		push!(seam, j)
	end
...
function seam_from_precomputed_least_energy(energies, starting_pixel::Int)
	...
	greedy_seam(least_energies, starting_pixel)
end
...
# ╔═╡ 8da58c0e-5c48-11eb-3242-1b6df8e02784
...
# ╔═╡ 17999de6-5c4f-11eb-0efa-e9a69a583c96
img_low = decimate(img, 2)

# ╔═╡ 4e3ef866-f3c5-11ea-3fb0-27d1ca9a9a3f
if shrink_dict
	dict_carved = shrink_n(img_low, 50, recursive_memoized_seam)
...
if shrink_matrix
	matrix_carved = shrink_n(img_low, 50, matrix_memoized_seam)
...
if shrink_bottomup
	...
	bottomup_carved = shrink_n(img_low, 50, seam_from_precomputed_least_energy)
...
# ╔═╡ 339c25ea-5c4a-11eb-24af-dd8550546509
memory
