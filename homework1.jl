...
function vecvec_to_matrix(vecvec)
	mat = Matrix{Number}(undef, length(vecvec[1]), length(vecvec))
...
function mean_colors(image)
	r = [i.r for i in image]
	g = [i.g for i in image]
	b = [i.b for i in image]
...
# ╔═╡ b46e05a6-2ac2-11eb-3f5b-1fd0a5e92a79
...
The image is unrecognisable with intensity above 2, because, as the amplitude of the noise grows it gets easier to get the cutoff values(0 and 1) and as you grow the values, more and more pixels get those maximum states.
"""

# ╔═╡ 9fe0e284-2ac4-11eb-3401-ddd981d92460
...
# ╔═╡ 92725820-2ac5-11eb-3478-95f32c9b9ced
extend(v, n)
...
# ╔═╡ eed5a79e-2ac6-11eb-1e90-add7185b3c21
@bind l Slider(0:10)

# ╔═╡ d9e955d6-2ac6-11eb-0d92-c3565cd725f3
...
function gaussian_kernel(n)
	...
	r = n^2
	l = (r - 1) ÷ 2
	G(x) =  (1/sqrt(2π*σ^2))*exp(-x^2/(2σ^2))
	gauss = G.(-l:l)
	return gauss./sum(gauss)
end

# ╔═╡ c0cea452-2acb-11eb-12bb-27934c53f77e
colored_line(gaussian_kernel(4))

# ╔═╡ f8bd22b8-ee14-11ea-04aa-ab16fd01826e
md"Let's test your kernel function!"

# ╔═╡ 2a9dd06a-ee13-11ea-3f84-67bb309c77a8
@bind gaussian_kernel_size_1D  Slider(2:10)# change this value, or turn me into a slider!
...
begin
	...
	function extend_mat_i(M::AbstractMatrix, i, j)
		r = extend_mat([Float64(p.r) for p in M], i, j)
		g = extend_mat([Float64(p.g) for p in M], i, j)
		b = extend_mat([Float64(p.b) for p in M], i, j)
		return RGB(r,g,b)
	end
end
...
begin
	...
	[extend_mat_i(philip_head, i, j) for (i,j) in Iterators.product(-50:size(philip_head,1)+51, (-50:size(philip_head,2)+51))]
end

# ╔═╡ 27b38108-2ad9-11eb-03b7-cd3be163c4ad
[p.r for p in philip_head]
...
begin
	function convolve_image(M::AbstractMatrix, K::AbstractMatrix)
		...
		for i=1:m, j=1:n
			s = 0
			for y=1:mk, x=1:nk
				s += extend_mat(M, i+y-ly-1, j+x-lx-1) * K[y,x]
			end
			new_M[i,j] = s
		end
		return new_M
	end
	function convolve_image_c(M::AbstractMatrix, K::AbstractMatrix)
		new_M = copy(M)
		m, n = size(M)
		mk, nk = size(K)
		ly, lx = (mk - 1) ÷ 2, (nk - 1) ÷ 2
		...
		for i=1:m, j=1:n
			r_s, g_s, b_s = 0, 0, 0
			for y=1:mk, x=1:nk
				r_s += extend_mat(R, i+y-ly-1, j+x-lx-1) * K[y,x]
				g_s += extend_mat(G, i+y-ly-1, j+x-lx-1) * K[y,x]
				b_s += extend_mat(B, i+y-ly-1, j+x-lx-1) * K[y,x]
			end
			new_M[i,j] = RGB(myclamp(r_s), myclamp(g_s), myclamp(b_s))
		end
		return new_M
	end
end
...
convolve_image_c(philip, K_test)
...
# ╔═╡ a067e7b4-2ada-11eb-1847-fdb0f232040b
function gauss_kernel_2D(n)
	σ = 1
	# r = n^2
	l = (n - 1) ÷ 2
	G(x,y) =  (1/sqrt(2π*σ^2))*exp(-(x^2+y^2)/(2σ^2))
	gauss = Matrix{Number}(undef, n, n)
	for i=1:n, j=1:n
		gauss[i,j] = G(-l+i-1, -l+j-1)
	end
	return gauss./sum(sum(gauss, dims=1), dims=2)
end

# ╔═╡ f474d5a6-2ada-11eb-32ba-dd1eb03cfd55
gauss_kernel_2D(3)

# ╔═╡ aad67fd0-ee15-11ea-00d4-274ec3cda3a3
function with_gaussian_blur(image)
	kernel = gauss_kernel_2D(10)
	return convolve_image_c(image, kernel)
end
...
# ╔═╡ 34ac7b5c-2adf-11eb-22d4-5933e67f8755
...
with_sobel_edge_detect([Gray(p.r) for p in process_raw_camera_data(sobel_raw_camera_data)])
